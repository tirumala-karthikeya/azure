...
describe('Classic Release', function () {
    ...
    it('should fail when azure not authorized', function(done: Mocha.Done) {
        ...
        assert.equal(tr.warningIssues.length, 0, "should have no warnings");
        assert.equal(tr.errorIssues.length, 1, "should have 1 error issue");
        assert.equal(tr.errorIssues[0], 'Status Code (https://nextech-systems.vsrm.visualstudio.com/d73144da-a092-4b3d-9155-9744b35b85cd/_apis/release/releases/13807?api-version=6.1-preview.8): 401', 'error issue output');
        done();
    });
